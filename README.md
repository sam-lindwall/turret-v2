# Vision-tracking robot (PID control on STM32, YOLO on Raspberry Pi)
...
only works if the moment the absolute position from the encoder is read is the same time the offset is computed. Previously, it wasn't: the offset was measured at frame capture, but the encoder was read after ~200 ms of inference. If the motor has already moved for ~200 ms towards the previous target when the new command is sent, the unintended result is sustained hunting. The solution was to interpolate the encoder back to the frame's capture timestamp from a telemetry history buffer. This eliminated the time between when the absolute angle is measured and when the pixel offset is computed.
